...
let this_channel_id = 1;

let roomId
const roomIdMatch = location.hash.match( /^\#?([a-zA-Z0-9-]+)?(\.)?(\d*)/ );
if ( roomIdMatch ) 
{
	const room_id_hash  = roomIdMatch[1];
	const room_id_dot   = roomIdMatch[2];
	const room_id_count = roomIdMatch[3];
	...
	{
		this_channel_id = parseInt( room_id_count );
		roomId = room_id_hash.slice( 0 , -1 ) + this_channel_id.toString();
	}
	else 
	{
		this_channel_id = 1;
  ...
  computed: 
    ...
    currentOutput() 
      ...
      return this.selectedOutput
		...
		this_channel_number()
		{
			return this_channel_id.toString();
		}
  },
  watch: {
    selectedInput: {
